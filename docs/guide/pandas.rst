...
*pandas* must be installed to use its functionality from within PubChemPy. The easiest
way is to use pip::
...
It is possible for ``get_compounds``, ``get_substances`` and ``get_properties`` to
...
An existing list of Compound objects can be converted into a dataframe, optionally
specifying the desired columns::
